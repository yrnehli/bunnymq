    ...
    <div className="flex flex-col gap-y-8">
      <div className="grid grid-cols-12 items-center justify-between">
        ...
        <div className="col-span-12 mt-1 flex md:col-span-2 md:mt-0 md:justify-end">
          <PurgeMessages queue={queue} onPurge={() => invalidateQueries()} />
      ...
      <div>
        <div className="mb-1 text-lg font-medium">Construct Message 🚧</div>
        <Editor
          height="400px"
      ...
      <div>
        <div className="mb-1 text-lg font-medium">Message Preview 💬</div>
        {output ? (
          <SyntaxHighlighter>{output}</SyntaxHighlighter>
        ) : (
          <div className="text-red-600">
            Invalid message construction, expected a non-empty string to be
            returned!
          </div>
        )}
      </div>
      <div>
        {["prod", "production"].includes(environment) && (
          <div className="mb-3 text-red-600">
            <span className="font-bold">Warning</span>: you are publishing this
            message to a <span className="font-bold">production</span>{" "}
            environment!
          </div>
        )}
        <Button
