...
export function NavigationBar({
  ...
  return (
    <nav className="flex items-center justify-between border-b p-2" {...props}>
    ...
    >
      <BreadcrumbList>
        {breadcrumbs.map((breadcrumb, i) => {
          ...
          return (
            <div className="flex items-center gap-x-2" key={breadcrumb.path}>
              <BreadcrumbItem>
                {!last ? (
                  <BreadcrumbLink to={breadcrumb.path} className="capitalize">
                    {breadcrumb.title}
                  </BreadcrumbLink>
                ) : (
